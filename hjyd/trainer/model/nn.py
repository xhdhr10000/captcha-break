...
def load_model_nn(alpha=1e-3):  # `cnn` up to now
    with tf.Graph().as_default() as graph:
        ...
        # First Convolutional Layer, input@(96, 30), output@(49, 15)
        ...
        # Second Convolutional Layer, input@(48, 15), output@(25, 8)
        ...
        # Third Convolutional Layer, input@(24, 8), output@(12, 4)
        ...
        fc_layer_weight = weight_variable([12 * 4 * 64, 1024])
        fc_layer_bias = bias_variable([1024])

        pool_layer3_flat = tf.reshape(pool_layer3, [-1, 12 * 4 * 64])
